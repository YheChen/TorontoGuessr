...
4. For a fresh database, run [backend/supabase/schema.sql](backend/supabase/schema.sql) in the Supabase SQL editor.
   ...
   Optional: [backend/supabase/add_user_accounts.sql](backend/supabase/add_user_accounts.sql) adds the `profiles` table behind optional player accounts, with a case-insensitive unique display name and a `purge_stale_anonymous_users()` helper for clearing anonymous accounts that never played. Supabase Auth acts as the identity provider only: this backend stays the sole reader and writer of application data via the service-role key, which is what keeps answer coordinates out of the browser. Accounts are always optional, so the game works unchanged until this is applied (the `/me` routes report the feature as unavailable). Set `SUPABASE_JWT_SECRET` on the backend only if your project still signs tokens with the legacy symmetric secret; asymmetric tokens are verified against the project's published JWKS with no secret needed.
