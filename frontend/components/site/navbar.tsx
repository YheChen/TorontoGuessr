...
import { Menu, X, Trophy, Info, Play, BarChart3, Users } from "lucide-react";
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                className={cn(
                  // px-2.5 through the md band, px-4 from lg. At exactly 768px,
                  // where these links first appear, the logo, the links and the
                  // right-hand group needed 749px of a 728px content box, so
                  // justify-between had no free space and the Play button spilled
                  // 21px past the container and was visibly clipped by the
                  // viewport edge. Trimming 12px from each of the four links
                  // recovers 48px, which clears it with room rather than exactly.
                  "relative rounded-full px-2.5 py-2 text-sm font-medium transition-colors lg:px-4",
                  active
                    ? "text-foreground"
